...
public class HUD : MonoBehaviour
{
    ...
    {
        int selectedAux = LevelManager.GetIndexOfAntibodySelected();
        List<AntiBody> auxList = LevelManager.GetAntibodyList();
        selected.GetComponent<Image>().color = auxList[selectedAux].color;

        int indexleft = (selectedAux - 1) % auxList.Count;
        if (indexleft < 0) indexleft = auxList.Count - 1;
        left.GetComponent<Image>().color = auxList[indexleft].color;
        right.GetComponent<Image>().color = auxList[Mathf.Abs((selectedAux + 1) % auxList.Count)].color;
    }
