...
public class Spawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        time += Time.deltaTime;
        GameObject temp = ChooseObj();
        // quando o tempo de spawn é atingido instancia o objeto selecionado
        if (time >= timeToSpawn)
        {
            Instantiate(temp, new Vector3(this.transform.position.x, UnityEngine.Random.Range(upBound.transform.position.y, bottonBound.transform.position.y), this.transform.position.z), this.transform.rotation);
    ...
    GameObject ChooseObj()
    {
        ...
        return objects[resp].gameObject;
    }
}
